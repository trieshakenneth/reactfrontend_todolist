...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	useEffect(() => {
		const loadData = async () => {
			let response = null;
			try {
				let url = `https://springbootmyproject.onrender.com/api/todo/${pageNumber - 1}/${pageSize}`;

				if(filter === 'Completed'){
					url = `https://springbootmyproject.onrender.com/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
				} else if(filter === 'Not Completed'){
					url = `https://springbootmyproject.onrender.com/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
				}
	...
	return (
      ...
      <div className="table-responsive">
        <table className="table table-striped">
          ...
          <tbody>
            {todos.map((todo) => (
              <tr className={todo.isCompleted ? 'table-success' : ''} key={todo.id}>
                <td>{todo.title}</td>
